...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<?php
	if ( has_post_thumbnail()  && !in_category('dresses') && !in_category('jewellery') ) :
	...
	<header class="entry-header">
		...
		<div class="entry-meta">
			<?php if (!in_category('dresses') && !in_category('jewellery') ) superhero_posted_on(); ?>
		</div><!-- .entry-meta -->
	</header><!-- .entry-header -->
	...
	<div class="entry-content">
		<?php the_content(); ?>
		<?php 
			if (!in_category('dresses') && !in_category('jewellery') ) {
				wp_link_pages( array( 'before' => '<div class="page-links">' . __( 'Pages:', 'superhero' ), 'after' => '</div>' ) ); 
			}
		?>
	</div><!-- .entry-content -->
